...
function toEditableBriefing(briefing, agencyInputs = []) {
    const refs = new Map();
...
function toBriefing(briefing) {
    const { dsa_tdb_references: _dsaTdbReferences, user_comments: _userComments, agency_inputs: _agencyInputs, ...base } = briefing;
    return base;
}
...
export function Briefing() {
    ...
    const [sharedBriefing, setSharedBriefing] = useState(null);
    const [loading, setLoading] = useState(false);
    const [sharedLoading, setSharedLoading] = useState(false);
    const [agencyName, setAgencyName] = useState("");
    const [agencyContact, setAgencyContact] = useState("");
    const [agencyDraft, setAgencyDraft] = useState(EMPTY_AGENCY_DRAFT);
    ...
    async function addAgencyContributor() {
        if (!sharedBriefing || !agencyName.trim())
            return;
        setSharedLoading(true);
        try {
            ...
            setSharedBriefing(shared);
            setEditedBriefing((current) => current ? { ...current, agency_inputs: shared.agency_inputs } : current);
            setAgencyName("");
            setAgencyContact("");
            setSharedError(null);
        }
    ...
    async function saveSharedEdits() {
        if (!sharedBriefing || !editedBriefing)
            return;
        setSharedLoading(true);
        try {
            const shared = await api.updateSharedBriefing(sharedBriefing.briefing_id, toBriefing(editedBriefing));
            setSharedBriefing(shared);
            setBriefing(shared.briefing);
            setEditedBriefing(toEditableBriefing(shared.briefing, shared.agency_inputs));
            setSharedError(null);
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { children: [_jsx("h1", { className: "font-serif text-2xl text-eu-ink font-semibold", children: "Parliamentary briefing" }), _jsx("p", { className: "text-sm text-eu-slate-500 mt-0.5", children: "Two-page factual briefing citing specific DSA / AI Act articles, ready for human editing and DOCX export." })] }), _jsxs("div", { className: "surface p-5 print:hidden", children: [_jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4", children: [_jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Requester" }), _jsx("input", { value: requester, onChange: (e) => setRequester(e.target.value), className: "select" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Constituency" }), _jsx("select", { value: constituency, onChange: (e) => setConstituency(e.target.value), className: "select", children: ["RO", "DE", "FR", "PL", "IT", "ES", "NL", "SK", ""].map((c) => (_jsx("option", { value: c, children: c ? `${flag(c)} ${c}` : "All / cross-border" }, c || "all"))) })] }), _jsx("div", { className: "flex items-end", children: _jsxs("div", { className: "flex w-full flex-wrap gap-2", children: [_jsx("button", { onClick: generate, disabled: loading, className: "px-4 py-2 rounded-md bg-eu-blue text-white text-sm font-medium hover:bg-eu-blue/90 disabled:opacity-50", children: loading ? "Generating…" : "Generate briefing" }), _jsx("button", { onClick: createSharedBriefing, disabled: loading, className: "px-4 py-2 rounded-md border border-eu-blue/30 bg-eu-blue/5 text-sm font-medium text-eu-blue hover:bg-eu-blue/10 disabled:opacity-50", children: "Create shared briefing" })] }) })] }), _jsxs("div", { className: "mt-4", children: [_jsxs("div", { className: "section-heading mb-2", children: ["Items to include ", selected.size > 0 ? `(${selected.size} selected)` : "(top 5 if none selected)"] }), _jsx("div", { className: "space-y-1.5 max-h-56 overflow-y-auto", children: videos.map((v) => (_jsxs("label", { className: "flex items-center gap-3 px-2 py-1 rounded hover:bg-eu-slate-50 cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: selected.has(v.metadata.video_id), onChange: (e) => {
                                                ...
                                                setSelected(s);
                                            } }), _jsxs("span", { className: "font-mono text-xs text-eu-slate-500 w-16 shrink-0", children: ["sev ", v.severity.score.toFixed(0)] }), _jsx("span", { className: "text-sm truncate", children: v.metadata.title }), _jsxs("span", { className: "text-xs text-eu-slate-500 ml-auto", children: [flag(v.metadata.language), " \u00B7 ", compactNumber(v.metadata.view_count)] })] }, v.metadata.video_id))) })] })] }), sharedLoading && (_jsx("div", { className: "surface p-4 text-sm text-eu-slate-600", children: "Updating shared briefing\u2026" })), sharedError && (_jsx("div", { className: "surface p-4 text-sm text-sev-critical", children: sharedError })), briefing && editedBriefing && (_jsxs("div", { className: "space-y-4", children: [sharedBriefing && agencyId && (_jsx(AgencyContributionPanel, { sharedBriefing: sharedBriefing, agencyId: agencyId, draft: agencyDraft, submitted: agencySubmitted, onChange: setAgencyDraft, onSubmit: submitAgencyInput })), sharedBriefing && !agencyId && (_jsx(SharedBriefingPanel, { sharedBriefing: sharedBriefing, agencyName: agencyName, agencyContact: agencyContact, onAgencyNameChange: setAgencyName, onAgencyContactChange: setAgencyContact, onAddAgency: addAgencyContributor })), _jsxs("div", { className: "mx-auto flex max-w-[984px] flex-wrap items-center justify-between gap-3 print:hidden", children: [_jsxs("div", { className: "surface-tight flex min-h-10 flex-wrap items-center gap-3 border-eu-blue/30 bg-eu-blue/5 px-4 py-2 text-sm text-eu-ink", children: [_jsx("span", { className: "rounded bg-eu-blue px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-white", children: "Edit mode" }), _jsx("span", { children: "Manual edits only. DOCX export uses this edited version; no AI runs after generation." })] }), _jsxs("div", { className: "flex gap-2", children: [_jsx("button", { onClick: () => navigator.clipboard.writeText(JSON.stringify(editedBriefing, null, 2)), className: "text-xs px-3 py-1.5 border border-eu-slate-200 rounded-md hover:bg-eu-slate-50", children: "Copy JSON" }), _jsx("button", { onClick: () => exportDocx(editedBriefing), className: "text-xs px-3 py-1.5 bg-eu-blue text-white rounded-md hover:bg-eu-blue/90", children: "Export DOCX" }), sharedBriefing && !agencyId && (_jsx("button", { onClick: saveSharedEdits, className: "text-xs px-3 py-1.5 border border-eu-blue/30 bg-eu-blue/5 text-eu-blue rounded-md hover:bg-eu-blue/10", children: "Save shared edits" }))] })] }), _jsx(BriefingDocument, { briefing: editedBriefing, onChange: setEditedBriefing })] }))] }));
}
function SharedBriefingPanel({ sharedBriefing, agencyName, agencyContact, onAgencyNameChange, onAgencyContactChange, onAddAgency, }) {
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    const ownerPath = `/briefing?shared=${sharedBriefing.briefing_id}`;
    return (_jsxs("section", { className: "surface mx-auto max-w-[984px] p-5 print:hidden", children: [_jsxs("div", { className: "mb-4 flex flex-wrap items-start justify-between gap-3", children: [_jsxs("div", { children: [_jsx("div", { className: "section-heading mb-1", children: "Authorized agency input" }), _jsx("h2", { className: "font-serif text-xl font-semibold text-eu-ink", children: "Shared policy briefing" }), _jsx("p", { className: "mt-1 text-sm text-eu-slate-600", children: "Invite authorized agencies to add evidence or case details. Their submissions are additive, attributed, and included in the briefing export." })] }), _jsx("button", { type: "button", onClick: () => navigator.clipboard.writeText(`${origin}${ownerPath}`), className: "text-xs px-3 py-1.5 border border-eu-slate-200 rounded-md hover:bg-eu-slate-50", children: "Copy owner link" })] }), _jsxs("div", { className: "grid gap-3 md:grid-cols-[1fr_1fr_auto]", children: [_jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Agency name" }), _jsx("input", { value: agencyName, onChange: (e) => onAgencyNameChange(e.target.value), placeholder: "e.g. EDMO Hub, national regulator", className: "select" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Contact label" }), _jsx("input", { value: agencyContact, onChange: (e) => onAgencyContactChange(e.target.value), placeholder: "analyst name or mailbox", className: "select" })] }), _jsx("div", { className: "flex items-end", children: _jsx("button", { type: "button", onClick: onAddAgency, disabled: !agencyName.trim(), className: "px-4 py-2 rounded-md bg-eu-blue text-sm font-medium text-white hover:bg-eu-blue/90 disabled:opacity-50", children: "Add agency" }) })] }), _jsx("div", { className: "mt-4 space-y-2", children: sharedBriefing.contributors.length === 0 ? (_jsx("div", { className: "surface-tight p-3 text-sm text-eu-slate-500", children: "No agencies invited yet." })) : (sharedBriefing.contributors.map((agency) => {
                    const path = `/briefing?shared=${sharedBriefing.briefing_id}&agency=${agency.agency_id}`;
                    return (_jsxs("div", { className: "surface-tight flex flex-wrap items-center justify-between gap-3 p-3 text-sm", children: [_jsxs("div", { children: [_jsx("div", { className: "font-semibold text-eu-ink", children: agency.agency_name }), _jsxs("div", { className: "text-xs text-eu-slate-500", children: [agency.contact_label || "No contact label", " \u00B7 ", agency.status] })] }), _jsx("button", { type: "button", onClick: () => navigator.clipboard.writeText(`${origin}${path}`), className: "text-xs px-3 py-1.5 border border-eu-slate-200 rounded-md hover:bg-eu-slate-50", children: "Copy agency link" })] }, agency.agency_id));
                })) })] }));
}
function AgencyContributionPanel({ sharedBriefing, agencyId, draft, submitted, onChange, onSubmit, }) {
    const agency = sharedBriefing.contributors.find((c) => c.agency_id === agencyId);
    if (!agency) {
        return (_jsx("section", { className: "surface mx-auto max-w-[984px] p-5 text-sm text-sev-critical print:hidden", children: "This agency link is not authorized for the shared briefing." }));
    }
    return (_jsxs("section", { className: "surface mx-auto max-w-[984px] p-5 print:hidden", children: [_jsxs("div", { className: "mb-4", children: [_jsx("div", { className: "section-heading mb-1", children: "Agency contribution" }), _jsxs("h2", { className: "font-serif text-xl font-semibold text-eu-ink", children: ["Add input as ", agency.agency_name] }), _jsx("p", { className: "mt-1 text-sm text-eu-slate-600", children: "Additive evidence and case details will be attributed to this agency and included in the policy briefing export." })] }), submitted && (_jsx("div", { className: "mb-4 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800", children: "Contribution submitted and added to the shared briefing." })), _jsxs("div", { className: "grid gap-3", children: [_jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Author" }), _jsx("input", { value: draft.author, onChange: (e) => onChange({ ...draft, author: e.target.value }), placeholder: agency.contact_label || agency.agency_name, className: "select" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Agency summary" }), _jsx("textarea", { value: draft.summary, onChange: (e) => onChange({ ...draft, summary: e.target.value }), className: "select min-h-24 resize-y" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Case details" }), _jsx("textarea", { value: draft.case_details, onChange: (e) => onChange({ ...draft, case_details: e.target.value }), className: "select min-h-24 resize-y" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Evidence links, one per line" }), _jsx("textarea", { value: draft.evidence_links, onChange: (e) => onChange({ ...draft, evidence_links: e.target.value }), className: "select min-h-20 resize-y font-mono text-xs" })] }), _jsxs("label", { className: "flex flex-col gap-1", children: [_jsx("span", { className: "text-[10px] uppercase tracking-wider text-eu-slate-500", children: "Evidence notes" }), _jsx("textarea", { value: draft.evidence_notes, onChange: (e) => onChange({ ...draft, evidence_notes: e.target.value }), className: "select min-h-20 resize-y" })] }), _jsx("div", { children: _jsx("button", { type: "button", onClick: onSubmit, className: "px-4 py-2 rounded-md bg-eu-blue text-sm font-medium text-white hover:bg-eu-blue/90", children: "Submit agency input" }) })] })] }));
}
