...
def transcribe_video(video_id: str, language: str) -> Transcript:
    demo = find_demo_by_id(video_id)
    if demo is None:
        raise RuntimeError(
            f"No pre-computed transcript for video_id={video_id} "
            "and live WhisperX integration is not configured in this build."
        )

    segments = demo["transcript_segments"]
